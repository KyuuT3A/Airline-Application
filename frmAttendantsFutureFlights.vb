Public Class frmAttendantsFutureFlights
    Private Sub LoadFutureFlights()
        Try
            ...
            '                                 "FROM TFlights " &
            '                                 "WHERE dtmFlightDate > GETDATE() AND intFlightID IN (SELECT intFlightID FROM TAttendantFlights WHERE intAttendantID = " & SelectedAttendantID & ")"
            ...
            '(frmAttendantsFutureFlights.vb page usp_GetUpcomingFlightsForAttendant)
            cmdSelect = New OleDb.OleDbCommand("usp_GetUpcomingFlightsForAttendant", m_conAdministrator)
            cmdSelect.CommandType = CommandType.StoredProcedure


            objParam = cmdSelect.Parameters.Add("@Attendant_ID", OleDb.OleDbType.Integer)
            objParam.Direction = ParameterDirection.Input
            objParam.Value = SelectedAttendantID

            drSourceTableFlights = cmdSelect.ExecuteReader
            lstResults.Items.Clear()
